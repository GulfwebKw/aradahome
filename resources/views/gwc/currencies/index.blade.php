...
<head>
    ...
    <meta charset="utf-8" />
    <title>{{ __('adminMessage.websiteName') }}|{{ __('adminMessage.currencies') }}</title>
                        ...
                        <div class="kt-container  kt-container--fluid ">
                            <div class="kt-subheader__main">
                                ...
                                <div class="kt-subheader__breadcrumbs">
                                    ...
                                    <a href="javascript:;"
                                        class="kt-subheader__breadcrumbs-link">{{ __('adminMessage.currencylisting') }}</a>

                                </div>
                            </div>
                            <div class="kt-subheader__toolbar">
                                <form class="kt-margin-l-20" method="get" id="kt_subheader_search_form"
                                    action="{{ url('gwc/currencies') }}">
                                ...
                                <div class="btn-group">
                                    @if (auth()->guard('admin')->user()->can('currency-create'))
                                        <a href="{{ url('gwc/currencies/create') }}"
                            ...
                            @if (auth()->guard('admin')->user()->can('currency-list'))
                                ...
                                <table class="table table-striped- table-bordered table-hover table-checkable "
                                    ...
                                    <thead>
                                        <tr>
                                            <th width="10">#</th>
                                            <th width="10">{{ __('adminMessage.currency_code') }}</th>
                                            <th width="10">{{ __('adminMessage.currency_symbol') }}</th>
                                            <th>{{ __('adminMessage.title') }}</th>
                                            <th width="10">{{ __('adminMessage.currency_rate') }}</th>
                                        ...
                                        @if (count($currencies))
                                            ...
                                            @foreach ($currencies as $key => $currency)
                                                <tr class="search-body">
                                                    ...
                                                    <td>{!! $currency->title_en !!}<br>{!! $currency->title_ar !!}</td>
                                                    <td>{{ $currency->rate }}</td>
                                                    <td><span class="kt-switch"><label><input
                                                                    {{ $baseCurrency == $currency->code ? 'disabled' : '' }}
                                                                    value="{{ $currency->id }}"
                                                                    {{ !empty($currency->is_active) ? 'checked' : '' }}
                                                                    type="checkbox" id="currencies"
                                                    ...
                                                    <td class="kt-datatable__cell">
                                                                    ...
                                                                    <ul class="kt-nav">
                                                                        @if (auth()->guard('admin')->user()->can('currency-edit'))
                                                                            <li class="kt-nav__item"><a
                                                                                    href="{{ url('gwc/currencies/' . $currency->id . '/edit') }}"
                                                                        ...
                                                                        @if (auth()->guard('admin')->user()->can('currency-delete'))
                                                                            @if ($baseCurrency !== $currency->code)
                                                                                <li class="kt-nav__item"><a
                                                                                        href="javascript:;"
                                                                                        data-toggle="modal"
                                                                                        data-target="#kt_modal_{{ $currency->id }}"
                                                                                        class="kt-nav__link"><i
                                                        ...
                                                        <!--Delete modal -->
                                                        <div class="modal fade" id="kt_modal_{{ $currency->id }}"
                                                                ...
                                                                <div class="modal-content">
                                                                    ...
                                                                    <div class="modal-footer">
                                                                        ...
                                                                        <button type="button" class="btn btn-danger"
                                                                            onClick="Javascript:window.location.href='{{ url('gwc/currencies/delete/' . $currency->id) }}'">{{ __('adminMessage.yes') }}</button>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
