...
<head>
    <meta charset="utf-8" />
    <title>{{__('adminMessage.websiteName')}}|Inventory Report</title>
            ...
            <div class="kt-content  kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">
                ...
                <div class="kt-subheader   kt-grid__item" id="kt_subheader">
                    <div class="kt-container  kt-container--fluid ">
                        <div class="kt-subheader__main">
                            <h3 class="kt-subheader__title">Inventory Report</h3>
                            <span class="kt-subheader__separator kt-subheader__separator--v"></span>
                            <div class="kt-subheader__breadcrumbs">
                                <a href="{{url('home')}}" class="kt-subheader__breadcrumbs-home"><i class="flaticon2-shelter"></i></a>
                                <span class="kt-subheader__breadcrumbs-separator"></span>
                                <a href="javascript:;" class="kt-subheader__breadcrumbs-link">Inventory Report</a>
                        ...
                        <div class="kt-subheader__toolbar">
                            ...
                            <form class="kt-margin-l-20" id="kt_subheader_search_form">
                                <div class="kt-input-icon kt-input-icon--right kt-subheader__search">
                                    <select class="form-control" name="inventory_id" onchange="$('#kt_subheader_search_form').submit();" id="selectInventory">
                                        @foreach(\App\Inventory::get(null , false) as $inventory)
                                            <option value="{{$inventory->id}}" @if ( $inventory->id == $inventorySelected ) selected @endif>{{$inventory->title}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="kt-input-icon kt-input-icon--right kt-subheader__search">
                                    <input type="text" class="form-control" placeholder="{{__('adminMessage.searchhere')}}" id="searchCat" name="searchCat" value="{{request()->get('searchCat') }}">
                ...
                <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
                    ...
                    <div class="kt-portlet kt-portlet--mobile">
                        ...
                        @if(auth()->guard('admin')->user()->can('inventory-list'))
                            <!--begin: Datatable -->
                                    ...
                                    @if(count($products))
                                        ...
                                        @foreach($products as $key=>$product)
                                            <tr class="search-body">
                                                ...
                                                @php
                                                    $price = ($product->countdown_price) ?: ($product->retail_price);
                                                @endphp
                                                <td>
                                                    {!!
                                                        $product['inventoryTitle'] .' → ' .$product['quantities'].
                                                        ' <br>' .
                                                        $price * $product['quantities'] . \App\Currency::default()
                                                    !!}
                                                </td>
                                        ...
                                        @endforeach
                                        <tr><td colspan="8" class="text-right"> <h3>Total Items : {{ $totalProducts }} , Total Price : {{ number_format($totalAmount,3) }}</h3></td></tr>
