...
<head>
    ...
    <meta charset="utf-8" />
    <title>{{__('adminMessage.websiteName')}}| Blog post</title>
                        ...
                        <div class="kt-subheader__toolbar">
                            ...
                            <div class="btn-group">
                                @if(auth()->guard('admin')->user()->can('post-create'))
                                    <a href="{{url('gwc/blog/post/create')}}" class="btn btn-brand btn-bold"><i class="la la-plus"></i>&nbsp;{{__('adminMessage.createnew')}}</a>
                                @endif
                        ...
                        <!--begin: Datatable -->
                            <table class="table table-striped- table-bordered table-hover table-checkable " id="kt_table_1">
                                ...
                                <tr>
                                    ...
                                    <th width="100">{{__('adminMessage.views')}}</th>
                                ...
                                @if(count($posts))
                                    @foreach($posts as $post)
                                        <tr class="search-body">
                                            ...
                                            <td>
                                                {{ route('blog.home' , [ 'kw', 'en',  'p' => $post->id]) }}
                                            <br>
                                                {{ route('blog.show' , ['kw', 'en',  $post->id , $post->slug]) }}
                                            </td>
                                            <td>{!! $post->viewed !!}</td>
                                            <td>{!! $post->status !!}</td>
                                            <td>
                                            ...
                                            <td class="kt-datatable__cell">
                                                 ...
                                                 <ul class="kt-nav">
                                                     <li class="kt-nav__item"><a href="{{url('gwc/blog/comment/?p='.$post->id)}}" class="kt-nav__link"><i class="kt-nav__link-icon fa fa-comment"></i><span class="kt-nav__link-text">Comments</span></a></li>

                                                 @if(auth()->guard('admin')->user()->can('post-edit'))
                                                         <li class="kt-nav__item"><a href="{{url('gwc/blog/post/'.$post->id.'/edit')}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">{{__('adminMessage.edit')}}</span></a></li>
                                                     @endif
                                                     @if(auth()->guard('admin')->user()->can('post-delete'))
                                                         <li class="kt-nav__item"><a href="javascript:;" data-toggle="modal" data-target="#kt_modal_{{$post->id}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text">{{__('adminMessage.delete')}}</span></a></li>
                                                     @endif
                                                 </ul>
                                                 </div>
                                                 </div>
                                                ...
                                                <!--Delete modal -->
                                                <div class="modal fade" id="kt_modal_{{$post->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    ...
                                    @endforeach
                                    <tr><td colspan="8" class="text-center">{!! $posts->appends($_GET)->links() !!}</td></tr>
                                @else
