...
<html lang="en">
	...
	<head>
		...
		<meta charset="utf-8" />
		<title>{{__('adminMessage.websiteName')}}|{{__('adminMessage.brand')}}</title>
							...
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									...
									<div class="kt-subheader__breadcrumbs">
										...
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('adminMessage.brandlisting')}}</a>
								...
								<div class="kt-subheader__toolbar">
									...
									<div class="btn-group">
										...
										<a href="{{url('gwc/brand/create')}}" class="btn btn-brand btn-bold"><i class="la la-plus"></i>&nbsp;{{__('adminMessage.createnew')}}</a>
                                        @endif
									...
									<table class="table table-striped- table-bordered table-hover table-checkable " id="kt_table_1">
										<thead>
											<tr>
												...
												<th width="10">{{__('adminMessage.logo')}}</th>
                                                <th width="10">{{__('adminMessage.background')}}</th>
                                                <th width="10">{{__('adminMessage.home')}}</th>
                                        ...
                                        @foreach($brandLists as $key=>$brandList)
											<tr class="search-body">
												<td>{{$brandLists->firstItem() + $key}}</td>
												<td>{!! $brandList->title_en !!}</td>
												<td>{!! $brandList->title_ar !!}</td>
												<td>
                                                @if($brandList->image)
                                                <img src="{!! url('uploads/brand/thumb/'.$brandList->image) !!}" width="40">
                                                @endif</td>
                                                
                                                <td>
                                                @if($brandList->bgimage)
                                                <img src="{!! url('uploads/brand/thumb/'.$brandList->bgimage) !!}" width="40">
                                                @endif</td>
                                                <td>
                                                <span class="kt-switch"><label><input value="{{$brandList->id}}" {{!empty($brandList->is_home)?'checked':''}} type="checkbox"  id="brandhome" class="change_status"><span></span></label></span>
                                                </td>
												<td>
                                                <span class="kt-switch"><label><input value="{{$brandList->id}}" {{!empty($brandList->is_active)?'checked':''}} type="checkbox"  id="brand" class="change_status"><span></span></label></span>
                                                </td>
                                                ...
                                                <td class="kt-datatable__cell">
                                                 ...
                                                 <li class="kt-nav__item"><a href="javascript:;" data-toggle="modal" data-target="#kt_modal_{{$brandList->id}}" class="kt-nav__link"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text">{{__('adminMessage.delete')}}</span></a></li>
                                                 @endif
                                                 </ul>
                                                 </div>
                                                 </div>
                                                 ...
                                                 <!--Delete modal -->
 <div class="modal fade" id="kt_modal_{{$brandList->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										...
										<div class="modal-body">
											@if( count($brandList->products) == 0 )
											<h6 class="modal-title">{!!__('adminMessage.alertDeleteMessage')!!}</h6>
											@else
												<div class="text-danger">
													Can not delete brand! because this brand has active product!
												</div>
											@endif
										</div>
										@if( count($brandList->products) == 0 )
										<div class="modal-footer">
											...
											<button type="button" class="btn btn-danger"  onClick="Javascript:window.location.href='{{url('gwc/brand/delete/'.$brandList->id)}}'">{{__('adminMessage.yes')}}</button>
										</div>
										@endif
									</div>
								</div>
                                        ...
                                        @php $p++; @endphp
                                        @endforeach   
                                        <tr><td colspan="8" class="text-center">{{ $brandLists->links() }}</td></tr> 
                                        @else
