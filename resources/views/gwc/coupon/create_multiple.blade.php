			...
			<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--ver kt-page">
				...
				<div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor kt-wrapper" id="kt_wrapper">
					...
					<div class="kt-content  kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor" id="kt_content">
							...
							<div class="kt-container  kt-container--fluid ">
								<div class="kt-subheader__main">
									...
									<div class="kt-subheader__breadcrumbs">
										...
										<a href="javascript:;" class="kt-subheader__breadcrumbs-link">{{__('Create Multiple Coupon')}}</a>
									</div>
								</div>
						...
						<div class="kt-portlet__head kt-portlet__head--lg">
									<div class="kt-portlet__head-label">
										...
										<h3 class="kt-portlet__head-title">{{__('Create Multiple Coupon')}}</h3>
									</div>
					...
					@if(auth()->guard('admin')->user()->can('coupon-create'))
                         ...
                         <form name="tFrm"  id="form_validation"  method="post" action="{{route('storeMultipleCoupon')}}"
                          ...
                          <input type="hidden" name="_token" value="{{ csrf_token() }}">
											<div class="kt-portlet__body">
                                                ...
                                                <div class="form-group row">
													...
													<label class="col-2 col-form-label zonePrice " style="display: none;">Zone country</label>
													<div class="col-2">
														<span class="kt-switch zonePrice" style="display: none">
															<label>
																<input type="checkbox"  name="is_zone_free"  id="is_zone_free" value="1"/>
																<span></span>
															</label>
														</span>
													</div>
                                         ...
                                         <div class="form-group row">
                                                <div class="col-lg-2">
                                                <label>{{__('Enter No. of Coupons')}}</label>
                                                <input type="text" class="form-control @if($errors->has('qty_coupons')) is-invalid @endif" name="qty_coupons" value="{{old('no_of_coupons')}}" autocomplete="off" placeholder="{{__('Quantity')}}*" />
                                                               @if($errors->has('qty_coupons'))
                                                               <div class="invalid-feedback">{{ $errors->first('qty_coupons') }}</div>
                                                               @endif
                                                </div>
                                                <div class="col-lg-2">
                                                ...
                                                <option value="amt" @if(old('coupon_type') && old('coupon_type')=='amt') selected @endif>{{__('adminMessage.amountkd')}}</option>
                                                <option value="per" @if(old('coupon_type') && old('coupon_type')=='per') selected @endif>{{__('adminMessage.percentage')}}</option>
                                                </select>
                                                ...
                                                <input type="text" class="form-control @if($errors->has('coupon_value')) is-invalid @endif" name="coupon_value"
                                                               value="{{old('coupon_value')}}" autocomplete="off" placeholder="{{__('adminMessage.enter_coupon_value')}}*" />
                                                ...
                                                <label>{{__('adminMessage.date_range')}}</label>
													<div class="input-group">
														<input type="text" class="datepick form-control @if($errors->has('start_date')) is-invalid @endif" name="start_date" value="{{old('start_date')}}" autocomplete="off" placeholder="{{__('adminMessage.start_date')}}*" />
														<input type="text" class=" datepick form-control @if($errors->has('end_date')) is-invalid @endif" name="end_date"
                                                               value="{{old('end_date')}}" autocomplete="off" placeholder="{{__('adminMessage.end_date')}}*" />
													</div>
                                                ...
                                                <label>{{__('adminMessage.price_range')}}</label>
													<div class="input-group">
														<input type="text" class="form-control @if($errors->has('price_start')) is-invalid @endif" name="price_start" value="{{old('price_start')}}" autocomplete="off" placeholder="{{__('adminMessage.price_start')}}*" />
														<input type="text" class="form-control @if($errors->has('price_end')) is-invalid @endif" name="price_end"
                                                               value="{{old('price_end')}}" autocomplete="off" placeholder="{{__('adminMessage.price_end')}}*" />
													</div>
                                               ...
                                               <div class="form-group row">
                                                ...
                                                <input type="text" class="form-control @if($errors->has('usage_limit')) is-invalid @endif" name="usage_limit" value="{{old('usage_limit')}}" autocomplete="off" placeholder="{{__('adminMessage.enter_usage_limit')}}*" />
                                                ...
                                                <option value="web" @if(old('is_for') && old('is_for')=='web') selected @endif>{{__('adminMessage.web')}}</option>
                                                <option value="app" @if(old('is_for') && old('is_for')=='app') selected @endif>{{__('adminMessage.app')}}</option>
                                                </select>
