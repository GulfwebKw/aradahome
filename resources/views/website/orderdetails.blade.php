...
@section('title' , __('webMessage.orderdetails').' ( '.Request()->orderid.' ) ' )
@section('breadcrumb' )
    ...
    <li class="breadcrumb-item active">{{ __('webMessage.orderdetails')  }} ( {{Request()->orderid}} )</li>
@endsection
@section('breadcrumb_title2' )
    <a target="_blank" href="{{url(app()->getLocale().'/order-print/'.Request()->orderid)}}" class="btn btn-fill-out float-right"><i class="fas fa-print"></i>{{__('webMessage.print')}}</a>
@endsection
                ...
                @if(!empty($trackLists) && count($trackLists)>0)
                    ...
                    <div class="row" style="border:1px #CCCCCC solid;margin-top:20px;padding:20px;border-radius:5px;">
                        ...
                        <div class="col-xs-12 col-md-3 col-lg-2">
                            ...
                            <div>
                                @if(!empty($orderDetails->is_paid)) <font color="#009900">{{strtoupper(__('webMessage.paid'))}}</font>@else <font color="#FF0000">{{strtoupper(__('webMessage.notpaid'))}}</font> @endif
                            </div>
                        </div>
                        @php
                    ...
                    <div class="row" style="border:1px #CCCCCC solid;margin-top:20px;padding:20px;border-radius:5px;">
                        <div class="col-lg-12">
                            ...
                            <table class="table">
                                ...
                                <tr>
                                    ...
                                    <th style="border-top:1px solid #fff;" class="text-center">{{__('webMessage.subtotal')}}</th>
                                </tr>
                                </thead>
                                <tbody>
                                @if(!empty($orderLists) && count($orderLists)>0)
                                    ...
                                    @foreach($orderLists as $orderList)
                                        @php
                                            ...
                                            $sizeName =App\Http\Controllers\webCartController::sizeNameStatic($orderList->size_id,app()->getLocale());
                                            $sizeName = trans('webMessage.size').':'.$sizeName;
                                            }else{$sizeName='';}
                                            if(!empty($orderList->color_id)){
                                            $colorName =App\Http\Controllers\webCartController::colorNameStatic($orderList->color_id,app()->getLocale());
                                        ...
                                        <tr id="cart-{{$orderList->id}}">
                                            <td class="product-thumbnail">
                                                ...
                                                @else
                                                    <img src="{{url('uploads/no-image.png')}}" alt="@if(!empty($productDetails['title_'.app()->getLocale()])){{$productDetails['title_'.app()->getLocale()]}}@endif">
                                                @endif
                                            </td>
                                            ...
                                            <td class="product-price text-center" data-title="{{__('webMessage.subtotal')}}">
                                                {{\App\Currency::default()}} <span class="subtotal_result{{$orderList->id}}">{{$subtotalprice}}</span>
                                            </td>
                                        </tr>
                                        @php
                            ...
                            <div class="row">
                                        ...
                                        @if(!empty($TransDetails->id))
                                            ...
                                            <div class="row">
                                                ...
                                                @endphp
                                                    ...
                                                    <div class="col-lg-6">
                                                        <strong>{{trans('webMessage.result')}} : </strong> @if(!empty($TransDetails['presult'])) <font color="{{$color}}"> {{$TransDetails['presult']}} </font> @endif
                                                    </div>
                                                @if(!empty($TransDetails['udf2']))
                                                    <div class="col-lg-6">
                                                        <strong>{{trans('webMessage.amount')}} : </strong>  {{$TransDetails['udf2'].' '.\App\Currency::defaultCMS()}}
                                                    </div>
                                                @endif
                                                @if(!empty($TransDetails['amt_dollar']))
                                                    <div class="col-lg-6">
                                                        <strong>{{trans('webMessage.amount')}} : </strong> <font color="#009900">{{trans('webMessage.usd').''.$TransDetails['amt_dollar']}}</font>
                                                    </div>
                                                @endif
                                                <div class="col-lg-6">
                                                    <strong>{{trans('webMessage.date')}} : </strong> @if(!empty($TransDetails['created_at'])) {{$TransDetails['created_at']}} @endif
                                                </div>
                                            </div>
                                        @endif
                                    @endif
                                ...
                                <div class="col-md-6 col-lg-4">
                                        <table class="table">
                                            <tbody>
                                            <tr>
                                                <th class="product-price" data-title="{{__('webMessage.subtotal')}}">
                                                    {{strtoupper(__('webMessage.subtotal'))}}
                                                </th>
                                                <td>{{\App\Currency::default()}} {{$totalprice}}</td>
                                            </tr>
                                            @if(!empty($orderDetails->bundle_discount))
                                                <tr>
                                                    <th class="product-price" data-title="{{__('webMessage.bundles.BundleDiscount')}}">{{strtoupper(__('webMessage.bundles.BundleDiscount'))}}</th>
                                                    <td><font color="#FF0000">-{{\App\Currency::default()}} {{$orderDetails->bundle_discount}}</font></td>
                                                </tr>
                                                @php
                                            ...
                                            @if(!empty($orderDetails->seller_discount))
                                                <tr>
                                                    <th class="product-price" data-title="{{__('webMessage.seller_discount')}}">{{strtoupper(__('webMessage.seller_discount'))}}</th>
                                                    <td><font color="#FF0000">-{{\App\Currency::default()}} {{$orderDetails->seller_discount}}</font></td>
                                                </tr>
                                                @php
                                                    $totalprice=$totalprice-$orderDetails->seller_discount;
                                                @endphp
                                            @endif
                                            @if(!empty($orderDetails->coupon_code) && $orderDetails->coupon_amount > 0 )
                                                <tr>
                                                    <th class="product-price" data-title="{{__('webMessage.coupon_discount')}}">{{strtoupper(__('webMessage.coupon_discount'))}}</th>
                                                    <td><font color="#FF0000">-{{\App\Currency::default()}} {{$orderDetails->coupon_amount}}</font></td>
                                                </tr>
                                                @php
                                            ...
                                            @if(empty($orderDetails->delivery_charges))
                                                <tr>
                                                    <th class="product-price" data-title="{{__('webMessage.delivery_charge')}}">{{strtoupper(__('webMessage.delivery_charge'))}}</th>
                                            ...
                                            <tfoot>
                                            <tr>
                                                <th class="product-price" data-title="{{__('webMessage.grandtotal')}}">{{strtoupper(__('webMessage.grandtotal'))}}</th>
                            ...
                            <div class="col-lg-12">
                                <div class="row">
                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.name')}} :</strong> @if($orderDetails->name) {{$orderDetails->name}}@else -- @endif
                                    </div>
                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.email')}} :</strong> @if($orderDetails->email) {{$orderDetails->email}}@else -- @endif
                                    </div>
                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.mobile')}} :</strong> @if($orderDetails->mobile) {{$orderDetails->mobile}}@else -- @endif
                                    </div>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.country')}} :</strong> @if($orderDetails->country_id) {{$countryInfo['name_'.app()->getLocale()]}}@else -- @endif
                                    </div>

                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.state')}} :</strong> @if($orderDetails->state_id) {{$stateInfo['name_'.app()->getLocale()]}}@else -- @endif
                                    </div>
                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.area')}} :</strong> @if($orderDetails->area_id) {{$areaInfo['name_'.app()->getLocale()]}} @endif
                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.block')}} :</strong> @if($orderDetails->block) {{$orderDetails['block']}}@else -- @endif
                                    </div>
                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.street')}} :</strong> @if($orderDetails->street) {{$orderDetails['street']}}@else -- @endif
                                    </div>
                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.avenue')}} :</strong> @if($orderDetails->avenue) {{$orderDetails['avenue']}}@else -- @endif
                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.house')}} :</strong> @if($orderDetails['house']) {{$orderDetails['house']}} @else -- @endif
                                    </div>
                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.floor')}} :</strong> @if($orderDetails['floor']) {{$orderDetails['floor']}} @else -- @endif
                                    </div>

                                    <div class="col-xs-12 col-md-6 col-lg-4">
                                        <strong>{{__('webMessage.landmark')}} :</strong> @if($orderDetails->landmark) {{$orderDetails['landmark']}} @else -- @endif
                                    </div>
                        ...
                        @if(! $orderDetails->is_paid)
                            ...
                            <div class="row" style="border:1px #CCCCCC solid;margin-top:20px;padding:20px;border-radius:5px;">
                                <div class="col-lg-12">
                                    <form action="{{ route('order.details.pay' , ['locale' => App()->getLocale() , 'orderid' => $orderDetails->order_id_md5 ]) }}" method="POST">
                                        @csrf
                                        <h4>{{strtoupper(__('webMessage.payonline'))}}</h4>
                                        <p class="mb-3">{{__('webMessage.payonlinedescription')}}</p>
                                        @php
                                            $payments = App\Country::getGateways($orderDetails->country_id);
                                            $p=1;
                                        @endphp
                                        @if(count($payments) > 0)
                                                ...
                                                @foreach($payments as $payment)
                                                    ...
                                                    <div class="custome-radio">
                                                        ...
                                                        <label class="form-check-label" for="{{$payment}}">
                                                            <img src="{{url('uploads/paymenticons/'.strtolower($payment).'.png')}}" height="30" alt="{{__('webMessage.payment_'.$payment)}}">&nbsp;{{$paytxt}}
                                                        </label>
                                                    </div>
                                                    @php $p++;@endphp
                                                @endforeach
